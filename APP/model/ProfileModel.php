...
class ProfileModel extends Model{
    ...
    public function getOwnSubmits(array $input):array{
        ...
        try{
            ...
            $sql = "SELECT id_problem, name_probl, solution, success, moment FROM solutions WHERE id_user = (?) ORDER BY moment DESC LIMIT (?) OFFSET (?)";
            $connection = $this->connectionPool->getConnection();
            $stmt =  $connection->prepare($sql);
    ...
    public function getSubmits(array $input):array{
        ...
        try{
            $offset = ($input['page'] - 1) * self::$nrOfProbPerPage;
            $sql = "SELECT id_problem, name_probl, success, moment FROM solutions WHERE id_user = (?) ORDER BY moment DESC LIMIT (?) OFFSET (?)";
            $connection = $this->connectionPool->getConnection();
            $stmt =  $connection->prepare($sql);
            $stmt->bindValue(1, $input['id'], PDO::PARAM_INT);
            $stmt->bindValue(2, self::$nrOfProbPerPage, PDO::PARAM_INT);
